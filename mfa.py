...
def save_fixation_properties(parent_dir, path):
    ...
    # # Temporary
    # x2 = np.arange(-4, 5)
    # means2 = np.array([-105, -110, -120, -80, 15, 30, 25, 33, 50])
    # errs2  = np.array([9, 10, 10, 9, 5, 10, 12, 7, 15])

    # axs[4].errorbar(
    #     x2, means2,
    #     yerr=errs2,
    #     fmt='s--', capsize=5, color='red', label='Empirical Data'
    # )
...
def save_choice_patterns(parent_dir: str, path: str):
    ...
    prob_left_by_bin = first_fix_df.groupby('diff_bin_choice')['choice_is_left'].mean()
    bin_centers = first_fix_df.groupby('diff_bin_choice')['signed_diff'].mean()
    ...
    p_ff_toward = first_fix_df.groupby('diff_bin_abs')['toward_larger'].agg(['mean', sem]).reset_index()
...
def save_fixation_structure(parent_dir: str, path: str):
    ...
    nfix_summary = first_fix_df.groupby('diff_bin')['fix_num_rev'].agg(['mean', sem]).reset_index()
...
def save_value_modulated_fixation(parent_dir: str, path: str):
    ...
    # (6) Middle fixation duration vs |ΔV|
    mid_summary = middle_fix_df.groupby('diff_bin')['fix_dur'].agg(['mean', sem]).reset_index()
    mid_summary.columns = ['diff_bin', 'mean_fix_dur', 'sem_fix_dur']
    mid_summary['x'] = mid_summary['diff_bin'].astype(int)

    # (7) First fixation duration vs |ΔV|
    first_summary = first_fix_df.groupby('diff_bin')['fix_dur'].agg(['mean', sem]).reset_index()
    ...
    trial_durs = df.groupby('trial').agg({'signed_fix_dur': 'sum', 'signed_diff': 'first'}).reset_index()
    # integer bins from −4..4
    trial_durs['diff_bin'] = pd.cut(trial_durs['signed_diff'],
                                bins=np.arange(-4.5, 5, 1),
                                labels=np.arange(-4, 5))
    net_summary = trial_durs.groupby('diff_bin')['signed_fix_dur'].agg(['mean', sem]).reset_index()
